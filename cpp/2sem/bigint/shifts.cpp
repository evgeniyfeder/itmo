...
big_integer operator>>(big_integer const& a, int b) {
	...
	if ((size_t)b > a_adding.digits.size() * big_integer::type_bits)
		return big_integer(0);

	uint32_t ost = 0, ost_size = big_integer::type_bits - (b % big_integer::type_bits);
	big_integer result; result.digits.pop_back();

	for (size_t i = 0; i < a_adding.digits.size() - 1; i++) {
	...
	ost = a_adding.digits[a_adding.digits.size() - 1] >> (big_integer::type_bits - ost_size);
