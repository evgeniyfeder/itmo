...
void big_integer::add_with_shift(big_integer const& a, int shift) {
	...
	if (this->digits.get_size() >= a.digits.get_size() + shift) {
		for (size_t i = 0; i < a.digits.get_size(); i++) {
			uint64_t res = uint64_t(a.digits[i]) + this->digits[shift + i] + tmp;
			tmp = (res > big_integer::max_num);
			this->digits.set(shift + i, uint32_t(res));
		}

		for (size_t i = a.digits.get_size(); i < this->digits.get_size() - shift; i++) {
			uint64_t res = this->digits[shift + i] + tmp;
			this->digits.set(shift + i, uint32_t(res));
			tmp = res > big_integer::max_num;
		}
	}
	else if (size_t(shift) >= this->digits.get_size()) {
		for (int i = this->digits.get_size(); i < shift; i++)
			this->digits.push_back(0);
		for (size_t i = 0; i < a.digits.get_size(); i++)
			this->digits.push_back(a.digits[i]);
	}
	else {
		for (size_t i = shift; i < this->digits.get_size(); i++) {
			uint64_t res = uint64_t(a.digits[i - shift]) + this->digits[i] + tmp;
			tmp = (res > big_integer::max_num);
			this->digits.set(i, uint32_t(res));
		}

		for (size_t i = this->digits.get_size(); i < a.digits.get_size() + shift; i++) {
...
big_integer operator-(big_integer const& a, big_integer const& b) {
	...
	int res_sign;
	if (a.digits.get_size() >= b.digits.get_size()) {
		res_sign = 1;
		less = &b, more = &a;
	}
	else {
	...
	if (more->digits.get_size() == less->digits.get_size() && reserve == 1) {
		res_sign = -res_sign;
		result.digits.set(result.digits.get_size() - 1, uint32_t(big_integer::base - result.digits[result.digits.get_size() - 1]));
	}
...
big_integer big_integer::div_short(uint32_t b, uint32_t *ost) const {
	...
	result.digits.resize(digits.get_size());
	for (int i = int(digits.get_size()) - 1; i >= 0; i--) {
		uint64_t cur = digits[i] + carry * big_integer::base;
		result.digits.set(i, uint32_t(cur / b));
		carry = cur % b;
	}
	if (ost != nullptr)
		*ost = uint32_t(carry);
...
big_integer operator/(big_integer const& a, big_integer const& b) {
	...
	if (b > a)
		return big_integer();
	if (b.digits.get_size() == 1)
		return a.div_short(b.sign * b.digits[0], nullptr);
    big_integer result; result.digits.do_empty();

	int128_t divident = 0, divisor = 0, res = 0;
	big_integer cur_divident = a;
	int32_t prefix_len = b.digits.get_size() + 1;

	cur_divident.digits.push_back(0);
	divisor = (int128_t(b.digits[b.digits.get_size() - 1]) << 32) +
		int128_t(b.digits[b.digits.get_size() - 2]);
	for (int32_t i = cur_divident.digits.get_size() - 1; i - prefix_len + 1 >= 0; i--) {
