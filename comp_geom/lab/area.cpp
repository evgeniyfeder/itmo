...
static std::map<vec, int> p_map;
static std::unordered_map<int, vec> p_coord;

static std::vector<std::map<vec, int, CompareForG>> planar_g;
...
double count_s(std::vector<int> &vertices) {
   ...
   while (tmp != right) {
      if (tmp == vertices.size() - 1) {
         s += num * fabs(p_coord[vertices[0]].x - p_coord[vertices[tmp]].x) * (p_coord[vertices[0]].y + p_coord[vertices[tmp]].y) / 2;
         tmp = 0;
      }
      else {
         s += num * fabs(p_coord[vertices[tmp + 1]].x - p_coord[vertices[tmp]].x) * (p_coord[vertices[tmp + 1]].y + p_coord[vertices[tmp]].y) / 2;
         tmp++;
      }
   }
