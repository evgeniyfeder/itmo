...
struct diy_fp multiply(struct diy_fp x, struct diy_fp y) {
	uint64_t a, b, c, d, ac, bc, ad, bd, tmp;
	struct diy_fp r; uint64_t M32 = 0xFFFFFFFF;
	a = x.f >> 32; b = x.f & M32;
	c = y.f >> 32; d = y.f & M32;
...
struct diy_fp minus(struct diy_fp x, struct diy_fp y) {
	struct diy_fp res = {x.f - y.f, x.e};
	return res;
}
...
struct diy_fp get_ck(int exp) {
	int alpha = -59;
	int k = ceil((alpha - exp + 63) * D_1_LOG2_10);
	int index = 343 + k;

	const struct diy_fp ans = { powers_ten[index], powers_ten_e[index] };
	return ans;
}

void round_weed(char* buffer, int len,
...
void digit_gen(struct diy_fp Mp, struct diy_fp W, struct diy_fp delta,
	char* buffer, int* len, int* K) {
	char *tmp_buf_1 = buffer;

	uint32_t div; 
	int d, kappa; 
	struct diy_fp one, wp_w = minus(Mp, W);
	...
	*len = 0; kappa = 10; div = TEN9;
	while (kappa > 0) {
		...
		p1 %= div; kappa--; div /= 10;
		int tmp = (((uint64_t)p1) << -one.e) + p2;
		if (tmp <= delta.f) {
			 *K += kappa;
			 round_weed(buffer, *len, wp_w.f, delta.f, tmp, ((uint64_t)div) << -one.e);
			return;
		}
	}
	int64_t unit = 1;
	...
	*K += kappa;
	...
	round_weed(buffer, *len, wp_w.f * unit, delta.f, p2, one.f);
}
...
void dbl2str(const double *in, char *out_buf) {
	struct diy_fp w = {0};
	union dbl_converter conv;
	conv.d = *in;
	uint64_t v = conv.i;
	if (v & 0x8000000000000000 != 0) {
		*(out_buf++) = '-';
		v = v & ~0x8000000000000000;
	}
	else {
		*(out_buf++) = '+';
	}

	w.f = v & ~0xFFF0000000000000;
	w.e = ((v & 0x7FF0000000000000) >> 52);
	w.e = w.e - 1023 - 52;
	...
	struct diy_fp w_plus = w, w_minus = w;
	w_plus.f += 1 << 10;
	w_minus.f -= 1 << 10;
	struct diy_fp c_k = get_ck(w_plus.e + 64);
	struct diy_fp M_plus = multiply(w_plus, c_k), M_minus = multiply(w_minus, c_k), M = multiply(w, c_k);

	M_plus.f--;
	M_minus.f++;

	struct diy_fp delta = minus(M_plus, M_minus);

	int len, K;
	*(out_buf++) = '0';
	*(out_buf++) = '.';

	digit_gen(M_plus, M, delta, out_buf, &len, &K);
